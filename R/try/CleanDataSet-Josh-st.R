...
dat_full.light <- read_delim("raw/fromJosh/AB15_20131114_F256_positionInfo.light.txt",
                             " ", escape_double = FALSE, col_names = FALSE,
                             trim_ws = TRUE)

dat_full <- read_delim("raw/fromJosh/AB16_20131114_F256_positionInfo.txt",
                       " ", escape_double = FALSE, col_names = FALSE,
                       trim_ws = TRUE)
...
r <- subset(res, res$chr == "ref|NC_001133|")

g <- ggplot(r, aes(x=pos, y=weight)) + geom_bar(stat="identity")

g <- ggplot(r, aes(x=pos, y=weight)) + geom_point()

gg <- g + ggplot(r, aes(x=pos, y=-weight)) + geom_point()
